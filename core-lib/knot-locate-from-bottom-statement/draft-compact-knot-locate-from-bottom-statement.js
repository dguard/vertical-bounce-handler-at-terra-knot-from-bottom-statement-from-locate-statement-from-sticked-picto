...
function KnotFromBottomStatement(pencil) {
    ...
    that.throughCtx = function (ctx) {
        ...
        // keep
        knotFromBottomStatement.stickedAlbum.knotFrom('pointer-into-bottom-statement').at(4).addEventListener('click', function () {

        })
        knotFromBottomStatement.stickedAlbum.knotFrom('pointer-into-bottom-statement').at(3).addEventListener('click', function () {

        })
        knotFromBottomStatement.stickedAlbum.knotFrom('pointer-into-bottom-statement').at(2).addEventListener('click', function () {
            // keep
            // use connected knotLocate
            // dcsTar.rememberSample pointer-from-front
            // dcsTar.draw
        })
        knotFromBottomStatement.stickedAlbum.knotFrom('pointer-into-bottom-statement').at(1).addEventListener('click', function () {
            // keep
        })
        // keep !
        // from-bottom-statement
